-- SQL Advent Calendar - Day 24
-- Title: New Year Goals - User Type Analysis
-- Difficulty: hard
--
-- Question:
-- As the New Year begins, the goals tracker team wants to understand how user types differ. How many completed goals does the average user have in each user_type?
--
-- As the New Year begins, the goals tracker team wants to understand how user types differ. How many completed goals does the average user have in each user_type?
--

-- Table Schema:
-- Table: user_goals
--   user_id: INT
--   user_type: VARCHAR
--   goal_id: INT
--   goal_status: VARCHAR
--

-- My Solution:

-- There are 3 distinct user types: Free, Pro, Team 
-- There are 3 distinct goal_status: Completed, In Progress, Not Started
-- There are 9 distinct users

WITH completed_goals AS (
        SELECT   user_id, 
                 user_type, 
                 COUNT(goal_status) AS completed_goal_count
        FROM     user_goals
        WHERE    goal_status = 'Completed'
        GROUP BY user_type, user_id) 

SELECT  user_type, 
        ROUND(AVG(completed_goal_count), 2) AS average_completed_goals
FROM    completed_goals
GROUP BY user_type;
